...
class DGModel_final(DGModel_memcls):
    def __init__(self, pretrained=True, mem_size=1024, mem_dim=256, cls_thrs=0.5, err_thrs=0.5, den_dropout=0.5, cls_dropout=0.3, has_err_loss=False,patch_size=10000, log_para=1000):
...
if __name__== "__main__":
    ...
    dgm = DGModel_final(pretrained, mem_size, mem_dim, cls_thrs, err_thrs, den_dropout, cls_dropout, has_err_loss, patch_size, log_para)
    img1 = torch.randn(16,3,320,320)
    img2 = torch.randn(16,3,320,320)
    c_gt = torch.randn(16,1,20,20)
    dc1, dc2, c1, c2, c_err, loss_con, loss_err = dgm.forward_train(img1,img2, c_gt)
    print("Test End!")
